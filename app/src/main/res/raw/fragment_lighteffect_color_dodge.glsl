...
vec4 ColorDodgeBlend(vec4 secondSourceColor, vec4 sourceColor, float opacity)
{
    if(sourceColor.a > 0.0)
    {
        ...
        mediump vec3 result = secondSourceColor.rgb / (white - sourceColor.rgb);
...
void main()
{
    ...
    // r,g,b,a
    gl_FragColor = ColorDodgeBlend(secondSourceColor, sourceColor, alpha);
}
