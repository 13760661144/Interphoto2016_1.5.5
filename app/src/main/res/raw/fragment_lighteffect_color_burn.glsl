...
vec4 ColorBurnBlend(vec4 secondSourceColor, vec4 sourceColor, float opacity)
{
    if(sourceColor.a > 0.0)
    {
        mediump vec4 whiteColor = vec4(1.0);
        sourceColor.rgb = clamp(sourceColor.rgb / sourceColor.a, 0.0, 1.0);
        highp vec3 result = whiteColor.rgb - (whiteColor.rgb - secondSourceColor.rgb) / sourceColor.rgb;
...
void main()
{
    ...
    // r,g,b,a
    gl_FragColor = ColorBurnBlend(secondSourceColor, sourceColor, alpha);
}
