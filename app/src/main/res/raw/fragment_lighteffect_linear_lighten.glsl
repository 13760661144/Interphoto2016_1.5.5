...
vec4 LinearLightBlend(vec4 secondSourceColor, vec4 sourceColor, float opacity)
{
    if(sourceColor.a > 0.0){
        sourceColor.rgb = clamp(sourceColor.rgb / sourceColor.a, 0.0, 1.0);

        mediump vec3 result = secondSourceColor.rgb + 2.0 * sourceColor.rgb - vec3(1.0);
...
void main()
{
    ...
    // r,g,b,a
    gl_FragColor = LinearLightBlend(secondSourceColor, sourceColor, alpha);
}
