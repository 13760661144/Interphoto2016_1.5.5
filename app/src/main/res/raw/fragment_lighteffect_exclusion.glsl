...
vec4 ExclusionBlend(vec4 secondSourceColor, vec4 sourceColor, float opacity)
{
    if(sourceColor.a > 0.0){
        sourceColor.rgb = clamp(sourceColor.rgb / sourceColor.a, 0.0, 1.0);

        mediump vec3 result = sourceColor.rgb + secondSourceColor.rgb - 2.0 * sourceColor.rgb * secondSourceColor.rgb;
...
void main()
{
    ...
    // r,g,b,a
    gl_FragColor = ExclusionBlend(secondSourceColor, sourceColor, alpha);
}
