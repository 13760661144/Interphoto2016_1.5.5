...
public class ImageFile2
{
	...
	public synchronized Bitmap MakeBmp(Context context, int minW, int minH)
	{
		...
		if(m_data != null)
		{
			Bitmap temp = Utils.DecodeImage(context, m_data, m_degree, m_scale, minW, minH);
			if(temp != null)
			{
		...
		{
			Bitmap temp = Utils.DecodeImage(context, m_cachePath, m_finalDegree, -1, minW, minH);
			if(temp != null)
			{
