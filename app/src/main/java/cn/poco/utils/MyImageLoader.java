...
public class MyImageLoader
{
	...
	public MyImageLoader()
	{
		...
		HandlerThread thread = new HandlerThread("loadImage");
		thread.start();
		m_imgHandler = new Handler(thread.getLooper())
		{
			@Override
		...
		m_uiHanlder = new Handler()
		{
			...
			public void handleMessage(Message msg)
			{
				...
				{
					res.cb.onLoadFinished(res.bmp, res);
				}
			}
		};
	}
	...
	public void LoadBmp(final ImageView view, final Context context, final String key, String url, final int width, final int height)
	{
		LoadItem item = new LoadItem(view.hashCode() + "_" + key, url);
		loadBmp(item, new ImageLoadCallback()
		{
			@Override
			public void onLoadFinished(Bitmap bmp, Object res)
			{
				if(view != null && res instanceof LoadItem && (((LoadItem)res).key.equals(view.hashCode() + "_" + key)))
				{
			...
			{
				return MakeBmp2(context, res, width, height);
			}
		});
	}

	public void LoadCircleBmp(final ImageView view, final Context context, String url, final int width, final int height)
	{
		LoadItem item = new LoadItem(url, url);
		loadBmp(item, new ImageLoadCallback()
		{
			@Override
			public void onLoadFinished(Bitmap bmp, Object res)
	...
	public synchronized void releaseMem()
	{
		...
		m_imgHandler.getLooper().getThread().interrupt();
	}

	public synchronized void releaseMem1()
	{
		if(m_imgCache != null)
		{
			m_imgCache.evictAll();
	...
	public static Bitmap MakeRoundBmp(Context context, Object res, int width, int height, float px)
	{
		Bitmap out;
//		Bitmap temp = Utils.DecodeImage(context, res, 0, -1, width, height);
	...
	public static Bitmap MakeBmp2(Context context, Object res, int width, int height)
	{
		Bitmap out;
		if(res instanceof String)
		{
			...
			{
				InputStream stream = FileUtil.getAssetsStream(context, (String)res);
				res = Utils.InputStreamToByteArray(stream);
			}
		}
		Bitmap temp = Utils.DecodeImage(context, res, 0, -1, width, height);
		out = MakeBmpV2.CreateFixBitmapV2(temp, 0, 0, -1, width, height, Bitmap.Config.ARGB_8888);
		return out;
	}
