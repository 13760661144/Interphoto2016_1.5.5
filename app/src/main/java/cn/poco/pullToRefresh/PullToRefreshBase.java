...
{	
	...
	private State m_pullDownState = State.NONE;
		...
		switch(ev.getAction())
		{
			...
			case MotionEvent.ACTION_MOVE:
			{
				...
				if(Math.abs(deltaY) > m_touchSlop || isRefreshing())
				{
					...
					{
						m_isHandledTouchEvent = (Math.abs(getScrollY()) > 0 || deltaY > 0.5f);
						//						if(m_isHandledTouchEvent)
						//						{
						//							m_refreshableView.onTouchEvent(ev);
						//						}
					}
				}
				...
				x_distance += Math.abs(curX - x_last); 
				y_distance += Math.abs(curY - y_last); 
				x_last = curX; 
				y_last = curY; 
		...
		switch(event.getAction())
		{
			...
			case MotionEvent.ACTION_UP:
			{
				if(m_isHandledTouchEvent)
				{
					...
					{
						if(m_pullToRefreshEnabled && m_pullDownState == State.RELEASE_TO_REFRESH)
						{
							startRefreshing();
							handled = true;
						}
	...
	protected void pullHeaderLayout(float delta)
	{
		...
		if(m_pullToRefreshEnabled && !isRefreshing())
		{
			...
			{
				m_pullDownState = State.RELEASE_TO_REFRESH;
			}
			else
			{
				m_pullDownState = State.PULL_TO_REFRESH;
			}
	...
	protected void startRefreshing()
	{
		...
		m_pullDownState = State.REFRESHING; 
	...
	{
		return m_pullDownState == State.REFRESHING;
	}
	...
	protected void refreshRefreshableViewSize(int width, int height) {
		if (null != m_refreshableViewWapper) {
			LinearLayout.LayoutParams lp = (LinearLayout.LayoutParams) m_refreshableViewWapper.getLayoutParams();
			if (lp.height != height) {
				lp.height = height;
