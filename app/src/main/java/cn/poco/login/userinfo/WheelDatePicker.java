...
public class WheelDatePicker extends LinearLayout
{
	...
	private void Init()
	{
		...
		m_monthPicker.setGrivity(Gravity.RIGHT);
		m_monthPicker.setOnSelectListener(new PickerCtrl.OnSelectListener()
		{
			...
			{
//				m_curSelMonth = Integer.parseInt(item.substring(0, item.length() - 1));
				m_curSelMonth = index+1;
		...
		m_dayPicker.setOnSelectListener(new PickerCtrl.OnSelectListener()
		{
			...
			public void onSelected(int index, String item)
			{
//				m_curSelDay = Integer.parseInt(item.substring(0, item.length() - 1));
				m_curSelDay = index+1;
				if(m_listener != null)
				{
					m_listener.onChange(m_curSelYear, m_curSelMonth, m_curSelDay);
		...
		m_yearPicker.setOnSelectListener(new PickerCtrl.OnSelectListener()
		{
			...
			public void onSelected(int index, String item)
			{
//				m_curSelYear = Integer.parseInt(item.substring(0, item.length() - 1));
				m_curSelYear = index+MIN_YEAR;
	...
	protected void InitYears()
	{
		...
		{
			m_years.add(String.valueOf(i) + getResources().getString(R.string.year));
		}
	}
	...
	protected int InitMonths(int year)
	{
		...
		boolean isChinese = LoginOtherUtil.isChineseLanguage(getContext());
		for(int i = 1; i <= maxMonth; i++)
		{
			if(isChinese)
			{
				m_months.add(String.valueOf(i) + getResources().getString(R.string.month));
			}else{
				String month = "";
				switch(i){
					case 1:
	...
	protected void InitDays(int year, int month)
	{
		...
		{
			m_days.add(String.valueOf(i) + getResources().getString(R.string.day));
		}
	}
